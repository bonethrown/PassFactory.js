...
function(Utility, FieldSet, Barcode, Color) {
    ...
    function Pass(args) {
        ...
        if (args) {
            ...
            if (args.passTypeIdentifier) {
                this.passTypeIdentifier = args.passTypeIdentifier;
            }
            if (args.serialNumber) {
                this.serialNumber = args.serialNumber;
            }
            if (args.teamIdentifier) {
                this.teamIdentifier = args.teamIdentifier;
            }

            if (args.associatedStoreIdentifiers) {
                this.associatedStoreIdentifiers =
                    args.associatedStoreIdentifiers;
            }
            
            if (args.locations) {
                this.locations = args.locations;
            }
            if (args.relevantDate) {
                this.relevantDate = args.relevantDate;
            }
            ...
            if (args.logoColor) {
                this.logoColor = args.logoColor;
            }
            if (args.suppressStripShine !== undefined) {
                this.suppressStripShine = args.suppressStripShine;
            }

            if (args.authenticationToken) {
                this.authenticationToken = args.authenticationToken;
            }
            if (args.webserviceURL) {
                this.webServiceURL = args.webServiceURL;
            }
        }
    }
    
    Pass.prototype = {
        addHeaderField: function(name, args) {
            this.headerFields.addField(name, args);
        },
        removeHeaderField: function(name) {
            this.headerFields.removeField(name);
        },

        addPrimaryField: function(name, args) {
            this.primaryFields.addField(name, args);
        },
        removePrimaryField: function(name) {
            this.primaryFields.removeField(name);
        },

        addSecondaryField: function(name, args) {
            this.secondaryFields.addField(name, args);
        },
        removeSecondaryField: function(name) {
            this.secondaryFields.removeField(name);
        },

        addAuxiliaryField: function(name, args) {
            this.auxiliaryFields.addField(name, args);
        },
        removeAuxiliaryField: function(name) {
            this.auxiliaryFields.removeField(name);
        },

        addBackField: function(name, args) {
            this.backFields.addField(name, args);
        },
        removeBackField: function(name) {
            this.backFields.removeField(name);
        },
        ...
        toJSON: function(args) {
            ...
            var throwPropertyError = function(p) {
            ...
            if (this.relevantDate) {
                result.relevantDate = this.relevantDate;
            }
            ...
            if (this.headerFields.length > 0) {
                styleDict.headerFields = this.headerFields;
            }
            if (this.primaryFields.length > 0) {
                styleDict.primaryFields = this.primaryFields;
            }
            if (this.secondaryFields.length > 0) {
                styleDict.secondaryFields = this.secondaryFields;
            }
            if (this.auxiliaryFields.length > 0) {
                styleDict.auxiliaryFields = this.auxiliaryFields;
            }
            if (this.backFields.length > 0) {
                styleDict.backFields = this.backFields;
            }
            ...
            if (this.logoText) {
                result.logoText = this.logoText;
            }
            if (this.suppressStripShine) {
                result.suppressStripShine = this.suppressStripShine;
            }

            // Web service keys
            if (this.authenticationToken) {
                result.authenticationToken = this.authenticationToken;
            }
